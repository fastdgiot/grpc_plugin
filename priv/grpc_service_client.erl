...
-spec {{method}}(ctx:t(){{^input_stream}} | {{pb_module}}:{{input}}(){{/input_stream}}{{^input_stream}}, {{pb_module}}:{{input}}(){{/input_stream}} | grpc_client:options()) ->
    {{^output_stream}}{{^input_stream}}{ok, {{pb_module}}:{{output}}(), grpc:metadata()}{{/input_stream}}{{#input_stream}}{ok, grpc_client:stream()}{{/input_stream}}{{/output_stream}}{{#output_stream}}{{^input_stream}}{ok, grpc_client:stream()}{{/input_stream}}{{#input_stream}}{ok, grpc_client:stream()}{{/input_stream}}{{/output_stream}} | grpc_stream:grpc_error_response() | {error, any()}.
...
-spec {{method}}(ctx:t(){{^input_stream}}, {{pb_module}}:{{input}}(){{/input_stream}}, grpc_client:options()) ->
    {{^output_stream}}{{^input_stream}}{ok, {{pb_module}}:{{output}}(), grpc:metadata()}{{/input_stream}}{{#input_stream}}{ok, grpc_client:stream()}{{/input_stream}}{{/output_stream}}{{#output_stream}}{{^input_stream}}{ok, grpc_client:stream()}{{/input_stream}}{{#input_stream}}{ok, grpc_client:stream()}{{/input_stream}}{{/output_stream}} | grpc_stream:grpc_error_response() | {error, any()}.
{{method}}(Ctx{{^input_stream}}, Input{{/input_stream}}, Options) ->
    {{^output_stream}}{{^input_stream}}grpc_client:unary(Ctx, <<"/{{unmodified_service_name}}/{{unmodified_method}}">>, Input, ?DEF({{input}}, {{output}}, <<"{{message_type}}">>), Options){{/input_stream}}{{#input_stream}}grpc_client:stream(Ctx, <<"/{{unmodified_service_name}}/{{unmodified_method}}">>, ?DEF({{input}}, {{output}}, <<"{{message_type}}">>), Options){{/input_stream}}{{/output_stream}}{{#output_stream}}{{^input_stream}}grpc_client:stream(Ctx, <<"/{{unmodified_service_name}}/{{unmodified_method}}">>, Input, ?DEF({{input}}, {{output}}, <<"{{message_type}}">>), Options){{/input_stream}}{{#input_stream}}grpc_client:stream(Ctx, <<"/{{unmodified_service_name}}/{{unmodified_method}}">>, ?DEF({{input}}, {{output}}, <<"{{message_type}}">>), Options){{/input_stream}}{{/output_stream}}.
